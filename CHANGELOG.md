...
### Fixed
- Loading an old .guv file no longer turns preset lamps into custom lamps. Legacy files stored a preset only by its display name (e.g. "USHIO B1.5 (PREVIEW)") with preview-era photometry that no longer matches the current preset data, so such lamps loaded as "custom" and spawned spurious custom lamp definitions. Lamps are now re-linked to the real preset by their saved name — any preset's name, with or without a "(PREVIEW)"-style suffix — adopting the preset's current photometry with placement, scaling, and enabled state preserved
