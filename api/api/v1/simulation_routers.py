...
@router.post("/generate-report", tags=["Simulation"])
def generate_report(request: SimulationRequest):
    """
    Generate a CSV report for the given room configuration.

    This creates a guv-calcs Room from the request data, runs calculation if needed,
    and returns the CSV report generated by room.generate_report().
    """
    try:
        # Create Room
        logger.info("Creating room for report generation...")
        room = Room(
            x=request.room.x,
            y=request.room.y,
            z=request.room.z,
            units=request.room.units,
            precision=request.room.precision or 3,
            standard=request.room.standard or "ACGIH",
            enable_reflectance=request.room.enable_reflectance or False,
            reflectances=request.room.reflectances,
            air_changes=request.room.air_changes or 1.0,
            ozone_decay_constant=request.room.ozone_decay_constant or 2.5,
        )

        # Add reflectance settings if enabled
        if request.room.enable_reflectance:
            if request.room.reflectance_x_spacings:
                room.ref_manager.x_spacings = request.room.reflectance_x_spacings
            if request.room.reflectance_y_spacings:
                room.ref_manager.y_spacings = request.room.reflectance_y_spacings
            if request.room.reflectance_x_num_points:
                room.ref_manager.x_num_points = request.room.reflectance_x_num_points
            if request.room.reflectance_y_num_points:
                room.ref_manager.y_num_points = request.room.reflectance_y_num_points
            if request.room.reflectance_max_num_passes:
                room.ref_manager.max_num_passes = request.room.reflectance_max_num_passes
            if request.room.reflectance_threshold:
                room.ref_manager.threshold = request.room.reflectance_threshold
        ...
        # Run calculation
        logger.info("Running calculation for report...")
        room.calculate()

        # Generate report
        logger.info("Generating report...")
        csv_bytes = room.generate_report()
